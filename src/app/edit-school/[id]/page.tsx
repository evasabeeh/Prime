...
export default function EditSchoolPage() {
  ...
  const fetchSchoolData = useCallback(async () => {
    try {
      ...
      if (result.success) {
        ...
        setValue('contact', school.contact.toString());
        ...
        showToast.error(result.error || 'Failed to fetch school data');
        router.push('/schools');
      }
    } catch (error) {
      console.error('Error fetching school data:', error);
      showToast.error('Failed to load school data');
      router.push('/schools');
    } finally {
      setIsLoading(false);
    }
  }, [schoolId, setValue, router]);
  ...
  const removeImage = () => {
    ...
    setImagePreview(schoolData?.image || null);
  ...
  const onSubmit = async (data: SchoolFormData) => {
    ...
    const loadingToast = showToast.loading('Updating school...');
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-secondary-light to-secondary flex items-center justify-center">
      ...
      <div className="py-8">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-primary mb-2">Edit School</h1>
            <p className="text-gray-600">Update the details for {schoolData?.name}</p>
          </div>
                ...
                {imagePreview ? (
                  <div className="border-2 border-primary rounded-lg p-4 bg-secondary-light/30">
                    ...
                    <div className="mt-3 text-center">
                      <p className="text-sm text-primary font-medium">{selectedImage?.name || 'Current school image'}</p>
                      {selectedImage && (
